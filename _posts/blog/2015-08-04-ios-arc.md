...
当参数从方法返回时，会被放到 Autorelease Pool 中，Autorelease Pool 自己销毁的时候会把它里面的对象都顺手清理掉，这样就把事干得很干净了。但是 Autorelease Pool 的生命周期是多久呢？在 [Objective-C Boot Camp][3] 这篇文章中把 \_\_autoreleasing 对象的生命周期讲的比较清楚。
