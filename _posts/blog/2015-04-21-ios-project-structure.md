...
###Xcode 项目结构和磁盘文件结构对应
上面说了 Group 和 Folder Reference 各自的优缺点，在项目中，我习惯上也是不使用 Folder Reference，只用 Group，但是我又常常想让 Xcode 的目录结构能够跟磁盘文件的结构保持一致，这样让我在找代码文件的时候能够更清晰，为了达到这个目的我创建和管理代码文件时就要麻烦了一些。

创建 Group 的方式是这样的，创建时需要先在磁盘上创建对应的文件夹，再把文件夹拖进 Xcode 项目中对应的位置，并选择 Create groups for any added folders。这样磁盘上的结构和 Xcode 中的项目结构就是一一对应的。但是当你在 Xcode 项目中把一个文件从一个目录拖动到另外一个目录中时，它在磁盘上的物理位置并没有发生改变，这会造成混乱。所以，为了保持对应关系，当你要改变文件的目录时，你需要手动到 Finder 文件夹中去移动文件，再在 Xcode 项目中去删除相应的文件引用再重新添加到新的目录下。
